...
from shs.util.ridgeplot import ridgeplot
...
class RecorderBase:
    ...
    def log_image_grid(
        ...
        val_range = (np.min(x.numpy()), np.max(x.numpy()))
        if x is not None:
            if not NCHW:
                x = x.permute(0, 3, 1, 2)
    ...
    def _process_hooked_tensor(self, tensor, name):
        ...
        if flat.is_cuda:
            ...
            # As of torch 1.0.1.post2+nightly, float16 cuda summary ops are not
            # supported (convert to float32)
